...
==> Scaffolded. The kro instance (clusters/main-configs/clusters/${CLUSTER_NAME}.yaml) drives CAPI,
  ...
  - task add-kubeconfig CLUSTER=${CLUSTER_NAME}
  - The sops-gpg key is mirrored into ${NS} automatically by reflector (ensure 'task enable-sops-reflection'
    has been run once); the ${CLUSTER_NAME}-critical layer is gated until that secret is present.
EOF
