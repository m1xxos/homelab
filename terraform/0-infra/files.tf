...
resource "proxmox_virtual_environment_download_file" "talos_nocloud_template" {
  ...
  file_name               = "talos-${local.talos.version}-nocloud-amd64-template.img"
  url                     = "https://factory.talos.dev/image/${local.talos.image_hash}/${local.talos.version}/nocloud-amd64.raw.gz"
  decompression_algorithm = "gz"
  overwrite               = false
}
